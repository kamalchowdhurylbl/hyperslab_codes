...
async_term(void)
{
    ...
    func_log(__func__, "Success with Argobots finalize");
    ...
    if (fapl > 0) {
        ...
        if (status < 0) {
            ...
            if (async_instance_g->disable_implicit_file) {
                ...
                func_log(__func__, "set implicit mode to false (new fapl used)");
            }
        }
    }
    ...
    if (dxpl > 0) {
        ...
        if (status < 0) {
            ...
            if (async_instance_g->pause != is_pause) {
                ...
                func_log(__func__, "set pause async execution to false (new dxpl used)");
            }
        }
        ...
        if (status < 0) {
            ...
            if (async_instance_g->delay_time != 0 && async_instance_g->delay_time_env == false) {
                ...
                func_log(__func__, "set async delay time to 0 (new dxpl used)");
            }
        }
    } // End if dxpl
...
static herr_t H5VL_async_init(hid_t __attribute__((unused)) vipl_id)
{
    ...
    func_log(__func__, "ASYNC VOL init");
...
H5VL_async_term(void)
{
    ...
    func_log(__func__, "ASYNC VOL terminate");
...
execute_parent_task_recursive(async_task_t *task)
{
    ...
    func_log(__func__, "finished executing task");
}
...
async_attr_create_fn(void *foo)
{
    ...
    func_log(__func__, "execute success");
                  ...
                  hid_t dxpl_id, void **req)
{
    ...
    if (is_blocking) {
        ...
        func_log(__func__, "waiting to finish all previous tasks, SYNC MODE now!");
...
{   
    ...
    /*if(type==H5S_SEL_HYPERSLABS)
        ...
        fprintf(stderr,"A sequence of points is selected\n");
    else if(type==H5S_SEL_ALL)
        fprintf(stderr,"The entire dataset is selected\n");
    else if(type==H5S_SEL_NONE)
            ...
            if(task_iter->func==async_dataset_write_fn){
                ...
                if(task_iter->parent_obj == parent_obj){
                    ...
                    iter_args = task_iter->args;
                           ...
                           if(ndim==1)
                                {  // push_contiguous(ndim,start,count);
                                   ...
                                   /*  if(H5Smodify_select(file_space_id,H5S_SELECT_SET,iter_args->file_space_id)< 0)
                                    ...
                                    //fprintf(stderr,"%d\n",status);
                                    fprintf(stderr,"\n iter_mem_start=%lld iter_mem_count=%lld iter_file_start=%lld iter_file_count=%lld element_size=%lld\n",iter_mem_start[0],iter_mem_count[0],iter_file_start[0],iter_file_count[0],element_size);
                                   if(file_start[0]>=iter_file_start[0])
                                   {
                                       ...
                                       memcpy(new_buffer+iter_file_count[0]*element_size,buf+mem_start[0]*element_size,file_count[0]*element_size);
                                   }
                                   else{
                                       memcpy(new_buffer,buf+(file_start[0]*element_size),file_count[0]*element_size);
                                       memcpy(new_buffer+mem_count[0]*element_size,buf+(iter_file_start[0]*element_size),iter_mem_count[0]*element_size);
                                   ...
                                   */
                                   ...
                                   // iter_args->file_space_id=new_memspace;
                                    ...
                                    //iter_args->file_space_id=new_memspace;
                                    //start[0]=file_start[0];
                                    //iter_file_start=start;
                                    //iter_file_count=count;
                                    //fprintf(stderr,"\n iter file space mem_start=%lld mem_count=%lld file_start=%lld file_count=%lld element_size=%lld\n",start[0],count[0],start[0],count[0],element_size);
                                    
                                    //fprintf(stderr,"\n     after operation   start=%llu count=%llu\n",start[0],count[0]);
                     ...
                     /* else if(check_contiguous(iter_args->file_space_id,file_space_id,start,count))
                       {   return_val=1;
                           fprintf(stderr, "\n-------####contiguous####---------\n");
                           if(ndim==1)
                                {  // push_contiguous(ndim,start,count);
                                   ...
                                   //memcpy(new_buffer,buf,count[0]*element_size);
                                    ...
                                    status = H5Sselect_hyperslab (mem_space_id, H5S_SELECT_SET, start, NULL,  count, NULL);//iter_args->file_space_id=file_space_id;
